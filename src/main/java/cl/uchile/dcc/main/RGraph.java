...
import cl.uchile.dcc.tools.CustomTripleBoundary;
...
import static cl.uchile.dcc.tools.CommonNodes.*;
...
public class RGraph {
	...
	private final UpdateRequest duplicatesRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("normalisation/duplicates.ru") );
	private final UpdateRequest conjunctionRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("normalisation/conjunction.ru") );
	private final UpdateRequest disjunctionRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("normalisation/disjunction.ru"));
	private final UpdateRequest joinRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("join.ru"));
	private final UpdateRequest branchCleanUpRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("branchLabel/cleanUp.ru"));
	private final UpdateRequest branchUnionRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("branchLabel/branchUnion.ru"));
	private final UpdateRequest filterVarsRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("branchLabel/filterVars.ru"));
	private final UpdateRequest subgraphRule = UpdateFactory.read(getClass().getClassLoader().getResourceAsStream("subLabel.ru"));
	...
	public static RGraph table(Table table) {
		...
		ans.typeMap.put(tableRoot,tableNode);
		...
		while (iter.hasNext()) {
			...
			while (vars.hasNext()) {
				...
				ans.typeMap.put(bindingNode,bindNode);
	...
	public void wellDesignedOptional(RGraph arg1) {
		graph.add(Triple.create(root, rightNode, arg1.root));
		merge(arg1);
	}
	...
	public static RGraph group(OpGroup arg) {
		...
		ans.typeMap.put(root,groupByNode);
